// Command release builds the Rienda release artifacts.
//
// It cross-compiles the Rienda executable for every supported platform,
...
// their SHA-256 checksums into the dist directory. Every released binary
// embeds the version, commit and build date of the release through ldflags,
// and the release workflow uploads the resulting files to the GitHub release
// that triggered it.
package main
...
const (
	...
	commandPackage   = "./cmd/rienda"
	distDirName      = "dist"
	manifestFileName = "manifest.json"
	projectName      = "rienda"
	projectRepo      = "varavelio/rienda"
	// versionPackage is the import path of the package holding the build
	// metadata injected through ldflags.
...
// buildMetadata is the provenance embedded into every released binary.
type buildMetadata struct {
	// Version is the release version without the tag prefix.
	Version string

	// Commit is the git commit hash of the release.
	Commit string
...
// ldflags returns the linker flags that embed the metadata into a binary.
func (m buildMetadata) ldflags() string {
	return fmt.Sprintf(
		"-s -w -X %s.Version=%s -X %s.Commit=%s -X %s.Date=%s",
		versionPackage, m.Version,
		versionPackage, m.Commit,
		versionPackage, m.Date,
	)
}
...
func run(ctx context.Context) error {
	...
	metadata := detectBuildMetadata(ctx, root)
	...
	for _, buildTarget := range releaseTargets {
		artifact, err := buildArchive(ctx, root, distDir, buildTarget, metadata)
		if err != nil {
			return err
		}
		artifacts = append(artifacts, artifact)
	}
	if err := writeManifest(distDir, metadata.Version, artifacts); err != nil {
		return err
	}
...
// detectBuildMetadata resolves the provenance embedded into the binaries.
//
// The release workflow passes the version, the commit and the build date
// explicitly; the fallbacks keep local runs working without configuration.
func detectBuildMetadata(ctx context.Context, root string) buildMetadata {
	return buildMetadata{
		Version: detectVersion(ctx, root),
		Commit: firstNonEmpty(
			os.Getenv("RIENDA_COMMIT"),
			os.Getenv("GITHUB_SHA"),
			gitOutput(ctx, root, "rev-parse", "HEAD"),
			"unknown",
		),
		Date: firstNonEmpty(
			os.Getenv("RIENDA_DATE"),
...
//
// The release workflow sets RIENDA_VERSION explicitly; the fallbacks keep
...
//
//nolint:gosec // the compiler, the targets and the output paths belong to the builder.
func buildArchive(
	ctx context.Context,
	root, distDir string,
	buildTarget target,
...
//
//nolint:gosec // the paths are relative to the dist directory of the builder.
...
//
//nolint:gosec // the command is git and every argument is a fixed builder option.
