...
`.github/workflows/release.yaml`: it cross-compiles the archives with
`task release`, publishes the Homebrew formulas generated by `installers/brew`
into the `varavelio/homebrew-tap` repository, and pushes the multi-arch Docker
images to Docker Hub and GHCR. Every artifact embeds its version, commit and
