<#
.SYNOPSIS
  Rienda installer for Windows.

.DESCRIPTION
  Downloads, verifies, and installs the Rienda executable for Windows.
...
.EXAMPLE
  ...
  $env:VERSION = "vx.x.x"; irm https://get.varavel.com/rienda.ps1 | iex

.EXAMPLE
  # Install to a custom directory quietly
  $env:INSTALL_DIR = "$HOME\.local\bin"; $env:QUIET = "true"; irm https://get.varavel.com/rienda.ps1 | iex
...
.EXAMPLE
  ...
  powershell -ExecutionPolicy ByPass -Command "`$env:VERSION = 'vx.x.x'; irm https://get.varavel.com/rienda.ps1 | iex"

.NOTES
  Options (environment variables):
    VERSION      : Version to install (e.g., vx.x.x). Defaults to "latest".
    INSTALL_DIR  : Install directory. Defaults to "$env:LOCALAPPDATA\Programs\rienda".
    QUIET        : Set to "true" to suppress output.
#>

$ErrorActionPreference = "Stop"

$script:Repo = "varavelio/rienda"
$script:BinaryName = "rienda"
$script:InstallDir = if ($env:INSTALL_DIR) { $env:INSTALL_DIR } else { "$env:LOCALAPPDATA\Programs\rienda" }
...
function Show-Banner {
  if (-not $script:Quiet) {
    if ($script:UseColors) { Write-Host "Rienda - simple, declarative, multi-agent LLM harness" -ForegroundColor Blue }
    else { Write-Host "Rienda - simple, declarative, multi-agent LLM harness" }
  }
}
...
function Get-PlatformArch {
  ...
  switch ($arch.ToUpper()) {
    ...
    default {
      Write-Err "Unsupported architecture: $arch. Rienda publishes Windows amd64 and arm64 binaries."
      exit 1
    }
  }
}
...
function Install-Rienda {
  $script:TmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "rienda-install-$(Get-Random)"
  ...
  try {
    ...
    Write-Info "Installation complete. Restart your terminal and run 'rienda --help' to verify."
  } finally {
    Invoke-Cleanup
  }
}
